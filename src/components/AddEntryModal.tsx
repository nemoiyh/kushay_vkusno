...
import { IBarcode, ICheck, ISearch } from "./Icons";
...
export function AddEntryModal({
  ...
  customFoods: Food[];
  onSaveCustomFood: (f: Omit<Food, "id">) => Food;
}) {
  const editing = input.entry;
  const editingFood = editing ? findFood(editing.foodId, customFoods) : undefined;
  const initialFood = editingFood ?? input.food ?? (input.barcode ? undefined : undefined);
  ...
  const [cName, setCName] = useState(editing && !editingFood ? editing.name : "");
  ...
  const results = useMemo(() => {
    ...
    const list = q
      ? [...FOODS, ...customFoods].filter((f) => f.name.toLowerCase().includes(q) || f.cat.toLowerCase().includes(q))
      : [...FOODS, ...customFoods];
  ...
  const preview = useMemo(() => {
    ...
    if (tab === "db" && food) {
      const k = gg / 100;
      return { kcal: Math.round(food.kcal * k), p: round1(food.p * k), f: round1(food.f * k), c: round1(food.c * k) };
    }
    const k = gg / 100;
    return {
  ...
  const problem = !nameOk
    ? tab === "db" ? "Выберите продукт из списка" : "Укажите название (минимум 2 символа)"
    : !gramsOk ? "Вес порции — от 1 до 2000 г" : "";
      ...
      >
        ...
        {tab === "db" ? (
          <div className="mt-4">
            ...
            <div className="mt-2 max-h-52 overflow-y-auto rounded-xl border border-line">
              {results.length === 0 ? (
                <p className="px-4 py-6 text-center text-sm text-faint">Ничего не нашлось.</p>
              ) : (
                results.map((f) => {
                  ...
                  return (
                    <button
                      key={f.id}
                      onClick={() => { setFood(f); setGrams(String(f.unit?.grams ?? 100)); }}
                      className={`flex w-full items-center gap-3 border-b border-linesoft px-3.5 py-2.5 text-left transition-colors last:border-0 ${active ? "bg-leafwash" : "hover:bg-field"}`}
                    >
                      <span className={`grid size-5 shrink-0 place-items-center rounded-full border ${active ? "border-leaf bg-leaf text-paperink" : "border-line text-transparent"}`}>
            ...
            <button
              ...
              className="btn-press mt-2.5 flex w-full items-center justify-center gap-2 rounded-xl border border-line bg-field px-4 py-2.5 text-[13px] font-semibold text-soft hover:text-ink"
            >
              <IBarcode width={16} height={16} /> Сканировать штрихкод
            </button>
          </div>
        ) : (
          <div className="mt-4 grid grid-cols-2 gap-2.5 sm:grid-cols-5">
            ...
            {(
              ...
              <div key={label} className="col-span-1">
                ...
                <input className="field tabular-nums" inputMode="decimal" value={val} onChange={(e) => set(e.target.value)} />
              </div>
            ))}
            <div className="col-span-1 flex items-end pb-1 text-[11px] leading-tight text-faint">на 100 г продукта</div>
          </div>
        )}
        ...
        <div className="mt-4 rounded-xl border border-line bg-field/70 p-3.5">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            <input
              className={`field w-28 text-center text-base font-bold tabular-nums ${touched && !gramsOk ? "field-invalid" : ""}`}
              inputMode="decimal"
              value={grams}
            ...
            {quick.map((q) => (
              <button
                ...
                className={`btn-press rounded-lg border px-2.5 py-1.5 text-xs font-semibold tabular-nums ${
                  gramsOk && Math.round(g) === q ? "border-ink bg-ink text-paperink" : "border-line bg-card text-soft hover:text-ink"
                }`}
              >
                {q}
              </button>
        ...
        {touched && problem && <p className="mt-2 text-xs font-medium text-danger">{problem}</p>}
