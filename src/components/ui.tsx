  ...
  return createPortal(
    ...
    >
      ...
      <div
        role="dialog"
        aria-modal="true"
        className="relative w-full sm:max-w-xl h-[90dvh] sm:h-auto max-h-[92dvh] overflow-y-auto rounded-t-2xl sm:rounded-2xl border border-line bg-card hard-sm animate-rise"
        style={{ boxShadow: "0 24px 60px rgba(18,38,43,0.28)" }}
      >
...
export function ToastStack({ toasts, onDismiss }: { toasts: ToastItem[]; onDismiss: (id: string) => void }) {
  ...
  return (
    <div className="pointer-events-none fixed inset-x-3 bottom-20 z-[10001] flex flex-col items-center gap-2 sm:inset-x-auto sm:right-6 sm:bottom-6 sm:items-end">
      {toasts.map((t) => (
        <div
          key={t.id}
    ...
    if (this.state.error) {
      return (
        <div className="card mx-auto mt-10 max-w-md p-6 text-center">
          <p className="font-display text-base font-bold">Что-то пошло не так</p>
          <p className="mt-2 text-xs text-soft">{this.state.error.message}</p>
          <button
            onClick={() => {
